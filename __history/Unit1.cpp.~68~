...
//---------------------------------------------------------------------------
void __fastcall TForm1::TrackBar1Change(TObject *Sender)//трэкбар сопротивление
{
	EditR->Text=IntToStr(TrackBar1->Position/10)+" H";
	if(TrackBar1->Position>0) ImageW->Visible=true; else ImageW->Visible=false;
}
//---------------------------------------------------------------------------
void __fastcall TForm1::TrackBar2Change(TObject *Sender)//трэкбар Угол
{
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Button4Click(TObject *Sender)//кнопка стоп
{
	Button1->Tag=4;	
}
//---------------------------------------------------------------------------
void __fastcall TForm1::Button3Click(TObject *Sender)//кнопка сбросить
{
	Button1->Tag=3;
	Shape1->Left=647;
	Shape1->Top=200+cos(0*PI_2/90)*A_y;
...
void __fastcall TForm1::Button1Click(TObject *Sender)//кнопка сохранить
{
	if (EditL->Text=="") 
	{
		MessageBox(0,L"Введите данные во все поля",L"Предупреждение", MB_ICONWARNING);
		Button1->Tag=0;
	}
		else                                                                                            
			{
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Button2Click(TObject *Sender)//кнопка пуск
{
	if (Button1->Tag==0) MessageBox(0,L"Сохраните характеристики",L"Предупреждение", MB_ICONWARNING);
	else
	{
			...
			Label4->Caption="T(Период)="+FloatToStr(T)+" c";
			Label5->Caption="w(Циклическая частота)="+FloatToStr(w)+"рад/c";
				...
				for (double t=0;t<T;t+=0.01)
				{
					if(Button1->Tag==3 || Shape1->Left>646) break; 
					else
						{
							...
							Label6->Caption="Угол: "+FloatToStr(round(alpha))+" °";
							Sleep(10);
							if (Shape1->Left>646) continue;
